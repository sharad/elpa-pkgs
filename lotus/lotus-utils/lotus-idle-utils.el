...
(defun select-frame-set-input-focus-no-raise-p ()
  ...
  (and (advice--p (advice--symbol-function 'select-frame-set-input-focus))
       (advice-function-member-p #'quiet--select-frame
...
(defun select-frame-set-input-enable-raise ()
  ...
  (when (fboundp 'remove-function)
    (remove-function (symbol-function 'select-frame-set-input-focus)
                     #'quiet--select-frame)))
...
(defun select-frame-set-input-disable-raise ()
  ...
  (when (fboundp 'add-function)
    (add-function :override (symbol-function  'select-frame-set-input-focus)
                  #'quiet--select-frame)))
