;;; remote.el --- remote                             -*- lexical-binding: t; -*-

;; Copyright (C) 2022  sharad

;; Author: s <>
;; Keywords: convenience, abbrev, abbrev, abbrev, abbrev, abbrev, abbrev, abbrev, abbrev, abbrev, abbrev, calendar
...
;; (let ((str "/scp:$USER@$HOST:/path/to/dir")
;;       (regexs (list
...
;; (ido-is-tramp-root "/scp:$USER@$HOST:")
;; (ido-is-root-directory "/")

(defun lotus-all-remote-tramp-config ()
  (progn
    ...
    (when nil         ;TODO: Fix it failing while using command $ emacs-batch-build-update-packages
      ...
      (defun update-ssh-agent-1 (&optional force)
        (unwind-protect
            (save-excursion
              ...
              (if (and (getenv "SSH_AUTH_SOCK" (selected-frame))
                       (> (length (getenv "SSH_AUTH_SOCK" (selected-frame))) 0))
                  (progn
                    (unless (and (not force)
                                 (getenv "SSH_AUTH_SOCK")
                                 (string-equal (getenv "SSH_AUTH_SOCK")
                                               (getenv "SSH_AUTH_SOCK" (selected-frame))))
                      (when (and (getenv "SSH_AUTH_SOCK" (selected-frame))
                                 (> (length (getenv "SSH_AUTH_SOCK" (selected-frame))) 0))
    ...
    (progn
      ;; (testing
      (when nil
        (when (or (null (getenv "SSH_AUTH_SOCK"))
                  (not (string-equal (getenv "SSH_AUTH_SOCK")
                                     (getenv "SSH_AUTH_SOCK" (selected-frame))))
                  (not (shell-command-local-no-output "ssh-add -l < /dev/null")))
          (find-file-noselect (or (plist-get (car auth-sources) :source)
