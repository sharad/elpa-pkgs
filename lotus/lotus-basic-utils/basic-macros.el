...
;;                     (cond
;;                       ((or (equal (cl-first feature) 'or)
;;                            (equal (cl-first feature) 'and)
;;                            (equal (cl-first feature) 'progn))
;;                        `(,(cl-first feature) ,@(mapcar (lambda (f) `(require ',f nil t)) (cl-rest feature))))
;;                       (t feature))
;;                     `(require ',feature nil t))
;;            ,@(if (stringp (cl-first forms))
;;                  (cl-rest forms)
;;                  forms))))
             ...
             (if (consp feature)
                 (cond
                   ((or (equal (cl-first feature) 'or)
                        (equal (cl-first feature) 'and)
                        (equal (cl-first feature) 'progn))
                    `(,(cl-first feature) ,@(mapcar #'refine (cl-rest feature))))
                   (t feature))
                 `(featurep ',feature))))
    `(progn
       (when ,(refine feature)
