  ...
  (cond ((and (consp ineq)
         ...
         (let ((arg1 (cadr ineq))
           ...
           (cond ((memq (car ineq) '(calcFunc-gt calcFunc-geq))
                  `(calcFunc-eq ,arg1 (+ ,arg2 ,(occ-obj-gen-constant (car ineq)))))
                 ((memq (car ineq) '(calcFunc-lt calcFunc-leq))
                  `(calcFunc-eq ,arg2 (+ ,arg1 ,(occ-obj-gen-constant (car ineq))))))))
        ((listp ineq) (cons (occ-obj-cmp2eq (car ineq) property)
                            (mapcar #'(lambda (ineq) (occ-obj-cmp2eq ineq)) (cdr ineq))))
        (t ineq)))

(defun occ-normalize-ineqs (ineqs)
  ;; (let ((ineqs (mapcar #'caddr ineqs)))
  ;;   (occ-obj-order-ineqs-expr ineqs))
  (mapcar #'occ-obj-cmp2eq ineqs))
