                              ...
                              (cond ((and (listp ineq)
                                    ...
                                    ((listp ineq) (cons (reinstate-op-char (car ineq) op-char op-replacement)
                                                        (mapcar #'(lambda (ineq) (reinstate-op-char ineq op-char op-replacement))
                                                                (cdr ineq))))
                                    (t ineq))))
      (let ((ineq-str ineq-str))
        (dolist (x replace-alist)
          (setf ineq-str (replace-op-char ineq-str (car x) (cdr x))))
        (let ((ineq (math-read-expr ineq-str)))
          (dolist (x replace-alist)
            (setf ineq (reinstate-op-char ineq (car x) (cdr x))))
          ineq)))))
