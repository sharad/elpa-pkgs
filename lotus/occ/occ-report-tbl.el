;;; occ-report-tbl.el --- OCC Report table           -*- lexical-binding: t; -*-
...
(defun occ-clocktable-write-default (ipos tables params)
...
(defun occ-clocktable-with-content-note-write (ipos tables params)
  "Write out a clock table at position IPOS in the current buffer.
TABLES is a list of tables with clocking data as produced by
`org-clock-get-table-data-alt'.  PARAMS is the parameter property list obtained
from the dynamic block definition."
...
(defun org-dblock-table:clocktable-alt (params)
  ...
  (catch 'exit
    (let* ((scope (plist-get params :scope))
      ...
      (setq pos (point))
      (cond
        ((and scope (listp scope) (symbolp (cl-first scope)))
