...
- Need a system to remind when already
  + no task clocked in
  + wrong task clock in according to context, including unnamed task

- Some formalization of context for task and project
- and a system to define context dynamically and further modify it.
- With minimal user intervention, with minimal attention of user, else it will
  distract the flow of user on current project or work.
