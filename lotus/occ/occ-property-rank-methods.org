...
*** Template property of task
    #+begin_src emacs-lisp
      ...
      (cl-defmethod occ-rankprop ((obj occ-tsk)
        ...
        (let ((tsk (occ-obj-tsk obj)))
          (let ((_template_ (occ-get-property tsk
                                              '_template_)))
            (let ((_template_-time (if _template_
                                       (org-duration-string-to-minutes _template_)
                                     0))
                  (clocked-time   (occ-get-property tsk
                                                    'clock-sum)))
              (if (and (numberp clocked-time)
                       (numberp _template_-time)
                       (> _template_-time clocked-time))
                  (- _template_-time
                     clocked-time)
                0)))))
      
      (cl-defmethod occ-list-p ((prop (eql _template_)))
        "Is the property _TEMPLATE_ has VALUES in list."
         nil)
      
      (cl-defmethod occ-prop-elem-to-org   ((prop (eql _template_))
                                            value)
        "Return string representation for property _TEMPLATE_."
        nil)
      
      (cl-defmethod occ-prop-elem-from-org ((prop (eql _template_))
                                            value)
        "Return the Actual Object representation for property _TEMPLATE_."
        nil)
      
      (cl-defmethod occ-readprop-elem-from-user ((obj occ-tsk)
                                                 (prop (eql _template_)))
        "READ the value for property _TEMPLATE_."
        (let ((tsk (occ-obj-tsk obj)))
