;;; occ-skel.el --- occ skel                         -*- lexical-binding: t; -*-
...
(defun line-to-skeleton (line)
  `( > ,(trim-string line)  \n))

(defun buffer-content-to-skeleton ()
  (let ((lines (with-current-buffer "*make skeleton*"
                 ;; skip header
                 (goto-char (point-min))
                 (re-search-forward "^[^;]")
                 ...
                 (split-string (buffer-substring-no-properties (point) (point-max)) "\n"))))
    (apply #'append
           (mapcar #'line-to-skeleton
                   lines))))

(defun make-skeleton-in-finish ()
  (interactive)
  (eval `(define-skeleton nn-skeleton
           "Test"
           ,@(buffer-content-to-skeleton))))
